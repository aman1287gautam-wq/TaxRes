...
def calculate_stay(arr_str, dep_str, exc_fys, smart=False, is_citizen=True, is_pio=False, 
    ...
    for inv in arr_invalids:
        incompletes.append(f"Invalid Arrival Date: {inv}")
    for inv in dep_invalids:
        incompletes.append(f"Invalid Departure Date: {inv}")
...
st.markdown("**100% compliant with IT Act 1961** • Section 6(1A) Deemed • RNOR • Crew • Smart Pairing")
...
with col1:
    arr = st.text_area("Arrival Dates (space-separated)", height=200, 
                       placeholder="01/04/2024 15/07/2024 10/01/2025")
with col2:
    dep = st.text_area("Departure Dates (space-separated)", height=200, 
...
taxpayer_type = st.radio("Select Taxpayer Type:", 
                        ["Indian Citizen", "Person of Indian Origin (PIO)", "Foreign Citizen (neither)"], 
                        index=0)
...
not_taxed_abroad = st.checkbox("Not liable to tax in any foreign country", value=False, 
                               help="For Deemed Residency u/s 6(1A)", disabled=not is_citizen)
...
if calculate:
    if not arr.strip() or not dep.strip():
        ...
        with st.spinner("Calculating Residency Status..."):
            try:
                fy_list, fy_days, residency, reasons, total, warns, _, fy_trips, match_log, incompletes = calculate_stay(
                    arr, dep, emp_fys, smart, is_citizen, is_pio, is_coming_on_visit, 
...
if st.session_state.results:
    ...
    if r.get("incompletes"):
        with st.expander("Incomplete / Invalid Entries", expanded=True):
    ...
    # One-Tap Copy + Download Section
    st.success(f"**Total Days in India: {r['total']}**")

    colc1, colc2 = st.columns(2)

    with colc1:
        if st.button("📋 Copy Table for Excel (One Tap)", type="secondary", use_container_width=True):
            ...
            js = f"""
                <script>
                    navigator.clipboard.writeText(`{txt}`).then(() => {{
                        alert("✅ Table copied successfully!\\n\\nPaste it directly into Excel (Ctrl + V)");
                    }}).catch(() => {{
            ...
            st.success("✅ Copied to clipboard! Ready to paste in Excel.")
...
"""
        ...
        href = f'<a href="data:file/txt;base64,{b64}" download="Tax_Residency_Report_{datetime.now().strftime("%Y%m%d")}.txt" style="text-decoration:none;">⬇️ Download Full Report</a>'
...
st.markdown("---")
st.caption("Made with ❤️ by **Aman Gautam** | 8433878823")
