...
def calculate_stay(arr_str, dep_str, exc_fys, smart=False, is_citizen=True, is_pio=False, is_coming_on_visit=False, income_15l=False, not_taxed_abroad=False, is_crew=False, assume_missing_days=0, assume_status="Non-Resident"):
...
st.markdown("**100% compliant with IT Act 1961** • 6(1A) Deemed • 120-day • RNOR • Crew • Smart Pairing • By Aman Gautam (8433878823)")
...
taxpayer_type = st.radio("Select Taxpayer Type:", ["Indian Citizen", "Person of Indian Origin (PIO)", "Foreign Citizen (neither)"], index=0)
...
not_taxed_abroad = st.checkbox("Not liable to tax in any foreign country", value=False, help="For Deemed Residency u/s 6(1A)", disabled=not is_citizen)
...
if st.session_state.results:
    ...
    df = st.dataframe(data, use_container_width=True, hide_index=True, on_select="rerun", selection_mode="single-row", key="res_table")
    ...
    with colc1:
        ...
        st.button("📋 Copy Table for Excel", use_container_width=True)
        st.text_area("Copy below text → Ctrl+A then Ctrl+C", txt, height=140, key="copy_text")
...
"""
        ...
        href = f'<a href="data:file/txt;base64,{b64}" download="Tax_Residency_Report_{datetime.now().strftime("%Y%m%d")}.txt">⬇️ Download Full Report</a>'
