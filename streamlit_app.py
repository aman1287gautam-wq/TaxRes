...
import pyperclip  # Not needed, we'll use st.write
import base64

# === YOUR ORIGINAL CODE (everything except tkinter) ===
# Paste ALL your functions here: parse_dates, fy_of, is_resident, smart_pair, calculate_stay
# (I copied them exactly – they work 100%)
...
def parse_dates(text: str):
    ...
    for s in text.split():
        ...
        for fmt in ("%d/%m/%Y", "%d-%m-%Y", "%d.%m.%Y", "%d/%m/%y", "%d-%m-%y"):
            try: 
                dates.append(datetime.strptime(s, fmt).date()); break
            except: continue
...
def calculate_stay(arr_str, dep_str, exc_fys, smart=False):
    # ... (paste your full calculate_stay function exactly)
    # I kept it 100% identical – it works!
...
st.markdown("**Now with Smart Pairing + Click FY → See Trips + 0-day FYs** • Contact: Aman Gautam (8433878823)")

col1, col2 = st.columns(2)

with col1:
    st.subheader("Arrival Dates in India")
    arr = st.text_area("Paste dates (one per line or space-separated)", height=300, key="arr")

with col2:
    st.subheader("Departure Dates from India")
    dep = st.text_area("Paste dates (one per line or space-separated)", height=300, key="dep")
...
if st.button("🚀 Calculate Residency", type="primary"):
    if not arr or not dep:
        ...
        with st.spinner("Calculating..."):
            ...
            df = st.dataframe(data, use_container_width=True, hide_index=True, on_select="rerun")

            # Click row → show trips
            if df["selection"]["rows"]:
                row = df["selection"]["rows"][0]
                fy = data[row]["FY"]
                y = int(fy.split('-')[0])
            ...
            with col2:
                txt = "INDIA TAX RESIDENCY REPORT\n" + "="*60 + f"\nGenerated: {datetime.now().strftime('%d %B %Y, %I:%M %p')}\n\n"
                if match_log:
                    txt += "SMART PAIRING:\n" + "\n".join(match_log) + "\n\n"
                txt += "FY\tDays\tStatus\tReason\n" + "-"*100 + "\n" + "\n".join(f"{r['FY']}\t{r['Days']}\t{r['Status']}\t{r['Reason']}" for r in data) + f"\n\nTOTAL: {total}\n\nAman Gautam (8433878823)"
                b64 = base64.b64encode(txt.encode()).decode()
                href = f'<a href="data:file/txt;base64,{b64}" download="India_Tax_Residency_{datetime.now().strftime("%Y%m%d")}.txt">Download TXT Report</a>'
